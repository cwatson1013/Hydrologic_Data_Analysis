...
install_github('streampulse/StreamPULSE', force = TRUE)
...
ggplot(Potdat,
  ...
  geom_point() +
  scale_x_log10() +
  scale_y_log10() #these are C-Q plots where we are plotting concentration (C) against the flow (Q); pretty typical to do a log transform of both axes for C-Q plots because if we don't do that, the points will be clumped together and we likely won't be able to see much

```

Which of these rivers is more chemostatic?

> Chemostatic: concentration of the river does not change with discharge; Pootomac river is not chemostatic because as you get higher discharge amounts, you get higher concentraions; therefore Crane Creek would be considered chemostatic
...
dygraph(
  ...
  dyRangeSelector() #nitrate concentrations decrease as discharge increases

#can have diluting (negative slope) or flushing (positive slope) streams
#hysteresis: difference between rising and falling limbs of Q and C at the beginning and end of the storm
```
...
ggplot(PotomacStorm, aes(x = Flow_Inst, y = Nitrate_mgl, color = dateTime)) +
  geom_point() #beginning of storm is darker blue
#diluting stream; as time progresses, you have a rising limb of the storm, and concentration starts to go down, but doesn't go all the way down; example of counterclockwise hysteresis; at every single discharge, you have two concentrations; slope on rising limb (quickflow) is different than the slope on the falling limb (baseflow) which is why we have a hysteresis plot; in quickflow (overland flow), you have lower concentrations of nitrate whereas in baseflow (groundwater), you have higher concentrations of nitrate, so most of the nitrate is coming in from the groundwater

#diluting storms: 

#flushing storms: concentrations of nitrates increase because you are also flushing the nutrients with high amounts of water
...
>how much carbon they consume and produce; GPP (gross primary productivity): fixing carbon from the atmospherea and increasing the amount of C in the stream (producing oxygen) and ecosystem respiration - consume oxygen while fixing CO2

Why would we want to know the *GPP* and *ER* of a river reach?

>The amount of material in a stream is really important to determine the number of trophic levels and how many fish you can support
...
```{r}
Enodat <- request_data( #have to use request_data to get data from StreamPULSE
  sitecode = "NC_Eno",
  variables = c('DO_mgL'), #have to give it some variables that you want it to pull
  startdate = "2016-09-15", 
  enddate = "2016-09-20"
  ) #pulls in a list of 3 different objects; doesn't pull in a dataframe

Eno.lon <- Enodat[[2]]$lon #indexing data with the [[]]; calling the second value on the list, but only want longitude data, so use $lon to get just that information

Eno.DO <- Enodat[[1]] %>%
  spread(value = value, key = variable) %>%  #have to spread if you want them in different columns
  mutate(DateTime_Solar = convert_UTC_to_solartime(DateTime_UTC, Eno.lon)) #can convert UTC to local solartime (solar time is if you just thought about the time when the sun is out, like having a sun dial out)
...
> D~t = diffusion at a time step; P_t: productivity, which increase during the daytime because O2 is higher; respiration happens all the time, and will be more or less constant during the day and night
...
> Kernal density plot: draws boundaries around bounderies of where most of those points fall; smaller area is where you have 25% of the points, and the bigger area is where you have 75% of the points fall. This is net consumer because ER is greater than GPP and it is consuming more carbon than it is actually producing
