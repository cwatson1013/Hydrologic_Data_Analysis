...
* **Autocorrelation**: Data points are not independent from one another (i.e., the measurement at a given time point is dependent on previous time point(s)); need to take into account when dealing with timeseries analysis
...
* **Heteroscedasticity**: The variance of the time series is not constant over time; if the variance is not constant over time
...
print(ClearCreekPlot) #cyclical patterns - La Nina or El Nino patterns, means over larger time scales; we see clear seasonal patterns here
...
> not in this case because we wouldn't be able to predict the average discharge in a given day
...
> Yes, when you have a very regular time scale that eliminates seasonality; if you don't have seasonality for your data; decompose data into annual means which sometimes means you get a better pattern
...
2. A **trend** component, which quantifies the upward or downward progression over time. The trend component of a time series does not have to be monotonic. Does not have to be a unidirectional trend.
...
#need to have regularly spaced intervals; specified frequency to say that it's yearly, monthly, or weekly data
#use table(diff(dataframe$variable)) to get a table and see if there is a jump in the data/a missing date
```
...
```{r}
?stl #models with a moving window
# Generate the decomposition
ClearCreek_Decomposed <- stl(ClearCreek_ts, s.window = "periodic") #this is a list

# Visualize the decomposed series. 
plot(ClearCreek_Decomposed) #bar represents the scale of the y-axis; seasonal + trend + remainder = data; high remainder corresponds to the error not predicted by seasonal and trend; also high remainder corresponds to high discharge data
...
Note that the decomposition can yield negative values when we apply a seasonal adjustment or a trend adjustment to the data. The decomposition is not constrained by a lower bound of zero as discharge is in real life. Make sure to interpret with caution! Decomposition doesn't realize that we don't have negative discharge values, so it will predict negative values if we use the seasonal trend; numbers aren't actually raelistic
