...
> Variable 1: secchi disk transperancy (epi_nutr)

> Variable 2: mean temperature (within state.chag)

> Variable 3: DOC (within epi_nutr)

> Variable 4: mean depth (lakes_limno)

> Variable 5: Chlorophyll-a (within epi_nutr)
...
```{r}
# Wrangle data here.
LAGOSstate <- LAGOSdata$locus %>% select(lagoslakeid, state_zoneid, gnis_name)
LAGOSstatedata <- LAGOSdata$state %>% select(state_name, state_zoneid)
LAGOS_chemical <- LAGOSdata$epi_nutr %>% select(lagoslakeid, sampledate, chla, doc, tp, tn, secchi)
LAGOSdepth <- LAGOSdata$lakes_limno %>% select(lagoslakeid, meandepth)

#joining state datasets together
LAGOSstatejoin <- left_join(LAGOSstate, LAGOSstatedata, by = "state_zoneid")

#joining state and chemical data 
LAGOSdata_skinny <- left_join(LAGOSstatejoin, LAGOS_chemial, by = "lagoslakeid")

#joining chemical & state table to the depth information
LAGOSwrangled <- left_join(LAGOSdata_skinny, LAGOSdepth, by = "lagoslakeid") %>%
  mutate(sampleyear = year(sampledate), 
         samplemonth = month(sampledate), 
         season = as.factor(quarter(sampledate, fiscal_start = 12))) %>%
  drop_na(chla:secchi, meandepth)
...
LAGOSwrangled.final <- LAGOSwrangled %>%
  select(chla, doc, tp, tn, secchi, meandepth) %>%
  na.omit()

#correlation plot of 7 variables of interest
LAGOSwrangledcorr <- cor(LAGOSwrangled.final)

corrplot(LAGOSwrangledcorr, upper = "ellipse")
...
> Some variables are positively correlated with others while some variables are negatively correlated with other variables. For example, mean depth and doc are negatively correlated. Whereas tp and chlorophyll are positively correlated. 

2. Create two linear regression models (hint: function `lm`) to predict TN and TP using your five predictor variables. 

```{r}
#linear regression predicting TN
TNregression <- lm(tn ~ chla + doc + secchi + meandepth + state_name, data = LAGOSwrangled)
summary(TNregression)

#linear regression predicting TP
TPregression <- lm(tp ~ chla + doc + secchi + meandepth + state_name, data = LAGOSwrangled)
